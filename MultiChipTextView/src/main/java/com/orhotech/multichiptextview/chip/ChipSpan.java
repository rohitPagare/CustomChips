package com.orhotech.multichiptextview.chip;
...
import com.orhotech.multichiptextview.R;
...
public class ChipSpan extends ImageSpan implements Chip {
        ...
        if (mCachedSize == -1 && usingFontMetrics) {
            ...
            if (mMaxAvailableWidth != -1) {
                ...
                if (actualWidth > maxAvailableWidthMinusMargins) {
                    ...
                    while ((calculateActualWidth(paint) > maxAvailableWidthMinusMargins) && !mTextToDraw.isEmpty()) {
    ...
    private void drawText(Canvas canvas, float x, int top, int bottom, Paint paint, CharSequence text) {
        ...
        float adjustedY = top + (((float) height / 2) + ((-fm.top - fm.bottom) / 2));
    ...
    private void drawIconBitmap(Canvas canvas, float x, int top, int bottom, Paint paint) {
        ...
        float xInsetWithinCircle = (float) (height - bitmapCanvas.getWidth()) / 2;
        ...
        float yInsetWithinCircle = (float) (height - bitmapCanvas.getHeight()) / 2;
